function minLargestElement(A, B){
    let ans  = 0;
    let stateArray = [...A];
function minHeap() {
    this.heap = new Array();
...
minHeap.prototype.extractMin = function extractMin() {
    ...
    (this.heap[i*2 + 1] !== undefined && this.heap[i].n > this.heap[i*2 + 1].n) || 
    (this.heap[i*2 + 2] !== undefined && this.heap[i].n > this.heap[i*2 + 2].n))) {
    ...
    while(B > 0) {
        ...
        stateArray[i] = n;
        minHeapObj.insert(n + A[i], i);
        B--;
    }

    return Math.max(...stateArray);
	}